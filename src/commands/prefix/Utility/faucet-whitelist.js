...
const parametersResponse = {
  content:
    "Parámetros incorrectos. El formato es: **!whitelist <add o remove> <@rol>**",
  ephemeral: true,
};

module.exports = {
  structure: {
    name: "faucet-whitelist",
    description:
  ...
  run: async (client, message, args) => {
    ...
    if (option === "add") {
      const roleAdded = await AddRoleToList("whitelist", roleId);
      ...
      if (roleAdded)
        return message.reply({
          content: `El rol ${existRoleInGuild.toString()} fue agregado a la whitelist`,
          ephemeral: true,
        });
    } else if (option === "remove") {
      const roleRemoved = await RemoveRoleFromList("whitelist", roleId);

      if (!roleRemoved)
        return message.reply({
          content: "El rol que quieres eliminar no existe en la whitelist",
          ephemeral: true,
        });

      return message.reply({
        content: `El rol ${existRoleInGuild.toString()} fue eliminado de la whitelist`,
        ephemeral: true,
      });
    }
  },
